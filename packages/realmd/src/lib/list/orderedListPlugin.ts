  ...
  class {
    ...
    createDecorations(view: EditorView) {
      ...
      for (let { from, to } of view.visibleRanges) {
        syntaxTree(state).iterate({
          ...
          enter: (node) => {
            ...
            if (
              type.name === "ListItem" &&
              node.node.parent?.name === "OrderedList"
            ) {
              ...
              if (match) {
                ...
                const markerEnd =
                  line.from + indent.length + number.length + 1 + space.length; // +1 for the dot
                ...
                // Hide the original number marker using Decoration.replace
                decorations.push(
                  Decoration.replace({
                    inclusive: true,
      ...
      decorations.sort((a, b) => {
        ...
        // Access startSide through the value property
